...
static int
set_datapath(int argc, const char* argv[])
{
    OIIO_DASSERT(argc == 2);
    tool.datapath = argv[1];
    return 0;
}

static int
set_referencefile(int argc, const char* argv[])
{
    OIIO_DASSERT(argc == 2);
...
// utils - rawtoaces
bool rawtoaces_convert(
    const std::string& inputfilename,
    const std::string& outputfilename,
    const std::string& datapath,
    const std::string& wbmethod = "metadata",
    const std::string& matmethod = "spectral",
    float headroom = 6.0f)
{
    rawtoaces::ImageConverter converter;
    converter.datapath = datapath;
    if (wbmethod == "metadata") {
        converter.wbMethod = rawtoaces::ImageConverter::WBMethod::Metadata;
    }
    else if (wbmethod == "illuminant") {
        converter.wbMethod = rawtoaces::ImageConverter::WBMethod::Illuminant;
    }
    else if (wbmethod == "box") {
        converter.wbMethod = rawtoaces::ImageConverter::WBMethod::Box;
    }
    else if (wbmethod == "custom") {
        converter.wbMethod = rawtoaces::ImageConverter::WBMethod::Custom;
    }
    if (matmethod == "spectral") {
        converter.matrixMethod = rawtoaces::ImageConverter::MatrixMethod::Spectral;
    }
    else if (matmethod == "metadata") {
        converter.matrixMethod = rawtoaces::ImageConverter::MatrixMethod::Metadata;
    }
    else if (matmethod == "adobe") {
        converter.matrixMethod = rawtoaces::ImageConverter::MatrixMethod::Adobe;
    }
    else if (matmethod == "custom") {
        converter.matrixMethod = rawtoaces::ImageConverter::MatrixMethod::Custom;
    }
    converter.headroom = headroom;
    // configure
    if (!converter.configure(inputfilename)) {
        print_error("failed when trying to configure converter for: ", inputfilename);
        return false;
    }
    // load
    if (!converter.load(inputfilename)) {
        print_error("failed when trying to load image data for: ", inputfilename);
        return false;
    }
    // process
    if (!converter.process()) {
        print_error("failed when trying to process image data for: ", inputfilename);
        return false;
    }
    // save
    if (!converter.save(outputfilename)) {
        print_error("failed when trying to save image data for: ", outputfilename);
        return false;
    }
    return true;
}
...
main( int argc, const char * argv[])
{
    ...
    ap.arg("--inputfile %s:FILENAME")
      .help("Input file")
      .action(set_inputfile);
    
    ap.arg("--datapath %s:DATAPATH", &tool.datapath)
      .help("Input data path for cmd, camera, illumant and training data")
      .action(set_datapath);
    ...
    if (ap.parse_args(argc, (const char**)argv) < 0) {
        print_error("could not parse arguments: ", ap.geterror());
        print_help(ap);
        ap.abort();
        return EXIT_FAILURE;
    }
    ...
    if (!tool.inputfile.size()) {
        print_error("must have input filename");
        ap.briefusage();
        ap.abort();
        return EXIT_FAILURE;
    }
    if (!tool.datapath.size()) {
        print_error("must have data path");
        ap.briefusage();
        ap.abort();
        return EXIT_FAILURE;
    }
    if (!tool.referencefile.size()) {
        print_error("must have reference file parameter");
        ap.briefusage();
        ap.abort();
        return EXIT_FAILURE;
    }
    if (argc <= 1) {
        ap.briefusage();
        print_error("incorrect number of parameters, for detailed help: it8tool --help");
        return EXIT_FAILURE;
    }
    
    // it8 program
    print_info("it8tool -- a set of utilities for computation of color correction matrices from it8 charts");
    ...
    // chart
    {
        ...
        if (rawtoaces_convert(tool.inputfile, swapfile, tool.datapath))
        {
